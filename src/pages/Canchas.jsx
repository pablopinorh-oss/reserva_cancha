...
function Canchas({ canchas, reservas, usuarioActual, cargarDatos }) {
  ...
  const reservarCancha = async (cancha, horario) => {
    ...
    // APLICACIÓN DEL RLS: Inserción con usuario_id y .select()
    const { error } = await supabase.from("reservas").insert([
      {
        ...
        horario,
        estudiante,
        usuario_id: usuarioActual.id, // Dato crítico para que la base de datos apruebe la reserva
        estado: "Activa",
      },
    ]).select();
  ...
  const cancelarReserva = async (reserva) => {
    ...
    // La eliminación/cancelación también pasará por el RLS automáticamente
